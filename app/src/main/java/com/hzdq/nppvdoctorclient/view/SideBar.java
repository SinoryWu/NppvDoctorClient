...
public class SideBar extends View {
     ...
     * @param attrs
     */
    public SideBar(Context context, @Nullable AttributeSet attrs) {  //要实现带有两个参数值的构造方法
        super(context, attrs);
        paint = new Paint();
        paint.setTextSize(40);  //字母字体大小
        paint.setColor(Color.WHITE); //默认设置画笔是白色
        paint.setAntiAlias(true); //设置抗锯齿
        paint.setTypeface(Typeface.DEFAULT);//设置粗体字
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i<words.length;i++){
            if (touchIndex == i){
                ...
                paint.setColor(Color.parseColor("#1FBE99"));
            }else {  //没有-1走下面代码
                //设置为白色(松开)
